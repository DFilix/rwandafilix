...
video_players.forEach((video_player) => {
  ...
  // play/pause when clicking the backsapce
  ...
  // Sync UI when video starts or stops
  ...
  mainVideo.addEventListener("loadeddata", (e) => {
    ...
    totalDuration.innerHTML = formatTime(videoDuration);
  });

  // Current video time update
  mainVideo.addEventListener("timeupdate", (e) => {
    let currentVideoTime = e.target.currentTime;
    current.innerHTML = formatTime(currentVideoTime);
  ...
  // Update video time on progress bar interaction
  ...
  function setTimelinePosition(e) {
    ...
    progress_Bar.style.width = `${
      (mainVideo.currentTime / videoDuration) * 100
    }%`;
    current.innerHTML = formatTime(mainVideo.currentTime);
  }
